...
class FistulaAnalyzer:
    ...
    def analyze(self, item) -> int:
        ...
        # TODO: уберём потом, когда не нужно будет сигнал на части бить
        record = cut_record(record, 2)

        # INFO: филтрация сигнала от разговоров и т д
        record = butter_bandpass_filter(
            record, self.lowcut, self.highcut, self.FREQ, self.filter_order
        )
